      ...
      if (!user) {
        ...
        for (let i = 0; i < user_collateral.length; i++) {
          if (
            ...
            user_collateral[i].collateral_amount = user_collateral[i].collateral_amount - req.body.loan_amount;
            break;
          }
        }
    ...
    query3
      .then(async (loan) => {
        if (!loan) {
          ...
          if (present) {
            new_funders[i].funding_amount += req.body.funded_amount;
          } else {
            new_funders.push({
    ...
    query4
      .then(async (lender) => {
        if (!lender) {
          ...
          if (present) {
            new_funded_loans[i].funding_amount += req.body.funded_amount;
          } else {
